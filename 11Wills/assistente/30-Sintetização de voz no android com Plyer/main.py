...
class Assistente(Screen):
    ...
    bot = Chatbot('Jujuzinho')

    def mensagem(self, msg, *args):

        self.ids.box.add_widget(Tarefa(text=msg))
        self.frase = self.bot.escuta(frase=msg)
        self.resposta = self.bot.pensa(self.frase)
        self.bot.fala(self.resposta)
        self.resp = Tarefa(text='Digitando...')
        self.ids.box.add_widget(self.resp)
        self.ids.texto.text = ' '
        Clock.schedule_once(self.responder, 5)

    def responder(self, *args):
        self.resp.text = self.resposta
        tts.speak(self.resposta)
