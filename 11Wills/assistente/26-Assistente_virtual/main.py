...
class Assistente(Screen):
    ...
    def mensagem(self, msg, *args):
        ...
        self.ids.box.add_widget(Assiste(text=msg))
        frase = self.bot.escuta(frase=msg)
        resposta = self.bot.pensa(frase)
        self.bot.fala(resposta)
        self.ids.box.add_widget(Assiste(text=resposta))
        self.ids.texto.text = ' '
